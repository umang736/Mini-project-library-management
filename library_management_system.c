...
void deletebooks()    //function that delete items from file fp
{
	...
	if(findbook=='t' )
	{
		...
		fclose(fp);
		remove("Umang.dat");
		rename("test.dat","Umang.dat"); //copy all item from temporary file to fp except that
		fp=fopen("Umang.dat","rb+");    //we want to delete
		if(findbook=='t')
		{
		    gotoxy(10,10);
...
void searchbooks()
{
    ...
    gotoxy( 15,20);
    printf("Enter Your Choice");
    fp=fopen("Umang.dat","rb+"); //open file for reading propose
			...
			if(checkid(t)==0) //issues those which are present in library
			{
			   ...
			   struct tm *current;
				...
				time(&timenow);
   				...
   				a.issued.dd=current->tm_mday;
   				a.issued.mm=current->tm_mon+1;
   				a.issued.yy=current->tm_year+1900;
   			     gotoxy(10,12);	
